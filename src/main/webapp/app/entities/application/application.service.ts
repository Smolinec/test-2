...
export class ApplicationService {
  ...
  protected convertDateFromClient(application: IApplication): IApplication {
    const copy: IApplication = Object.assign({}, application, {
      timestamp: application.timestamp && application.timestamp.isValid() ? application.timestamp.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((application: IApplication) => {
        application.timestamp = application.timestamp ? moment(application.timestamp) : undefined;
      });
    }
    return res;
  }
